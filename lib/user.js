...
const createUser = async (userData) => {
  return await UtilsLib.immediateExpr(async () => {
    ...
    const isAdmin = await userOps.isUserAdmin(email);
...
const createUserFromReqData = async (body) => {
  ...
  if (! await userOps.userByEmail(email)) {
    ...
    // hooksLib.applyHook('createUserFromReqData', newUser);
...
const getUsersName = async () => {
  const users = await userOps.allUsers();
  const usersWithNameAndPic = users.map(user => ({
    name: user.name,
    profilePicture: user.profilePicture // Assuming each user object has a 'profilePic' property
  }));

  const groupedNames = usersWithNameAndPic.reduce((acc, user) => {
  ...
  const sortedGroupedNames = Object.keys(groupedNames).sort().reduce((acc, key) => {
    acc[key] = groupedNames[key];
    return acc;
  }, {});
...
hooksLib.registerHook('createUserFromReqData', async (newUser) => {
  if (newUser) {
