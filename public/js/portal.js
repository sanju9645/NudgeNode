...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (async () => {
    const configDetails = await fetchConfigDetails();
    ...
    const handleInputField = (inputField, isValid = false) => {
      if (isValid) {
        inputField.classList.add('border-gray-300', `focus:border-${configDetails.BORDER_COLOR}-400`);
        ...
        inputField.classList.remove('border-gray-300', `focus:border-${configDetails.BORDER_COLOR}-400`);
      }
      updateSignInButton();
      updateSignUpButton();
    }
